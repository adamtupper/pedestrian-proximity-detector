...
flags.DEFINE_boolean('bounding_only', True, 'If True, generates bounding boxes '
...
def dict_to_tf_example(data,
  ...
  Args:
    data: dict holding the Supervisely annotation fields (with masks) for the
      image. For more information see https://docs.supervise.ly/ann_format/.
  ...
  # Load the image
  image_format = 'png'
  img_path = os.path.join(image_subdirectory, image_filename)
    ...
    for obj in data['objects']:
      ...
      if not bounding_only:
        ...
        right_pad = np.zeros((mask_height,
                              img_width - mask_width - mask_origin[0]))
        top_pad = np.zeros((mask_origin[1], img_width))
        bottom_pad = np.zeros((img_height - mask_height - mask_origin[1],
                               img_width))
...
def main(_):
  ...
  logging.info('Reading from persons dataset.')
